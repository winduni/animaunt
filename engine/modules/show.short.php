	...
	while ( $row = $db->get_row( $sql_result ) ) {
		...
		if( ! $row['category'] ) {
			...
			if( count( $cat_list ) == 1 OR ($view_template == "rss" AND $config['rss_format'] == 2)) {
				...
				$my_cat_link = implode( "<span>, </span>", $my_cat_link );
			}
		...
		if( count($xfields) ) {
			...
			foreach ( $xfields as $value ) {
				...
				if($value[0] == "link_carousel") {
					$id_y_a = explode("/", $xfieldsdata[$value[0]]);
					if(trim(end($id_y_a)) == "/")
						$id_y_s = trim($id_y_a[count($id_y_a)-2]);
					else
						$id_y_s = trim(end($id_y_a));
					$tpl->set( "{id-youtube}", $id_y_s);
					// $xfieldsdata[$value[0]] = str_replace("youtu.be/", "www.youtube.com/embed/", $xfieldsdata[$value[0]]);
				}
