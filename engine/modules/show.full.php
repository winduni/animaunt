...
if( ! defined( 'DATALIFEENGINE' ) ) {
	...
	if( ! $row['category'] ) {
		...
		if( count( $cat_list ) == 1 ) {
			...
			foreach ( $cat_list as $element ) {
               // if(in_array($element,array(14))) continue;
				...
				if( $element ) {
					...
					if( $config['allow_alt_url'] ) $my_cat_link[] = "<a class=\"ui-link\" href=\"" . $config['http_home_url'] . get_url( $element ) . "/\">{$cat_info[$element]['name']}</a>";
					else $my_cat_link[] = "<a href=\"$PHP_SELF?do=cat&amp;category={$cat_info[$element]['alt_name']}\">{$cat_info[$element]['name']}</a>";
				}
			}
				
			$my_cat_link = implode( "<span class=\"ui-link\">, </span>", $my_cat_link );
		}
	...
	if ( $row['id'] AND  $perm ) {
		...
		if( $config['allow_read_count'] AND !$news_page AND !$cstart) {
			if ( $config['allow_read_count'] == 2 ) {
				...
				if( !$readcount['count'] ) {
					...
					else $db->query( "UPDATE " . PREFIX . "_post_extras SET news_read=news_read+1, day_read=day_read+1 WHERE news_id='{$row['id']}'" );
       ...
       // $social_tags['description'] = $row['descr'];
		...
		$news_find = array ('{comments-num}' => number_format($row['comm_num'], 0, ',', ' '), '{views}' => $row['news_read'], '{category}' => $my_cat, '{link-category}' => $my_cat_link, '{news-id}' => $row['id'] );
		...
		if( $config['related_news'] AND $view_template != "print") {
			...
			if( $related_buffer === false ) {
				...
				while ( $related = $db->get_row() ) {
					...
					if(is_object($ifelse) && strpos( $tpl2->copy_template, "[if " ) !== false) {
						if(strtolower($config['charset']) == "utf-8")
							$tpl2->copy_template = preg_replace_callback("#\\[if (.+?)\\](.*?)\\[/if\\]#umis", array($ifelse, "StartIF"), $tpl2->copy_template);
						else
							$tpl2->copy_template = preg_replace_callback("#\\[if (.+?)\\](.*?)\\[/if\\]#mis", array($ifelse, "StartIF"), $tpl2->copy_template);
					}
					$tpl2->compile( 'content' );
		...
		if( $is_logged ) {
			...
			if( ! in_array( $row['id'], $fav_arr ) ) {
				...
				$tpl->set_block( "'\\[text-add-favorites\\](.*?)\\[/text-add-favorites\\]'si", "\\1" );
				$tpl->set_block( "'\\[text-del-favorites\\](.*?)\\[/text-del-favorites\\]'si", "" );
				$tpl->set_block( "'\\[del-favorites\\](.*?)\\[/del-favorites\\]'si", "" );
			} else { 
				$tpl->set_block( "'\\[text-add-favorites\\](.*?)\\[/text-add-favorites\\]'si", "" );
				$tpl->set_block( "'\\[text-del-favorites\\](.*?)\\[/text-del-favorites\\]'si", "\\1" );
		...
		$rand_banner = rand(1, 4);
		if($rand_banner == 1) {
			$tpl->set("{url_banner_1}", "https://aniga.me/pirateking/?esc=animaunt.ru_pk_940x220_1217");
			$tpl->set("{img_banner_1}", "http://animaunt.ru/templates/new_animaunt/images/pking_940x220.jpg");
		}
		elseif($rand_banner == 2) {
			$tpl->set("{url_banner_1}", "https://espritgames.ru/arpiel/promo/arp-firstlook/?esc=animaunt.ru_arpiel_940x220_0218");
			$tpl->set("{img_banner_1}", "http://animaunt.ru/templates/new_animaunt/images/arpiel_940x220.jpg");
		}
		elseif($rand_banner == 3) {
			$tpl->set("{url_banner_1}", "https://espritgames.ru/atlantida/?esc=animaunt.ru_ao_940x220_0218");
			$tpl->set("{img_banner_1}", "http://animaunt.ru/templates/new_animaunt/images/atlantida_940x220.jpg");
		}
		else {
			$tpl->set("{url_banner_1}", "https://espritgames.ru/dragonhunter/?esc=animaunt.ru_dh_940x220_1217");
			$tpl->set("{img_banner_1}", "http://animaunt.ru/templates/new_animaunt/images/dhunterx_940x220.jpg");
		}
		...
		$title_player = str_replace("&amp;amp;", "&amp;", htmlspecialchars( $row['title'], ENT_QUOTES, $config['charset'] ) );
		if( count($xfields) ) {
			...
			foreach ( $xfields as $value ) {
				...
				if ( $value[6] AND !empty( $xfieldsdata[$value[0]] ) ) {
					...
					foreach ($temp_array as $value2) {
						...
						$value2 = str_replace("&#039;", "'", $value2);
						if($value[0] == "dubers") {
							$value3[] = "<a class=\"ui-link\" href=\"" . $config['http_home_url'] . "voicers/". urlencode( $value2 ) . "/\">" . $value2 . "</a>";
						}
						else {
							if( $config['allow_alt_url'] )
					...
					if($value[0] == "dubers") {
						$xfieldsdata[$value[0]] = implode("<span class=\"ui-link\">, </span>", $value3);
					}
					else {
						$xfieldsdata[$value[0]] = implode(", ", $value3);
					}
				...
				if($value[3] == "imagegalery" AND $xfieldsdata[$value[0]] AND stripos ( $tpl->copy_template, "[xfvalue_{$value[0]}" ) !== false) {
					...
					foreach ($fieldvalue_arr as $temp_value) {
						...
						if($thumb_url) {
							...
							$gallery_image[] = "<a href=\"$img_url\" onclick=\"return hs.expand(this, { slideshowGroup: 'xf_{$row['id']}_{$value[0]}' })\" target=\"_blank\"><img src=\"{$thumb_url}\" alt=\"\" /></a>";
							$gallery_single_image['[xfvalue_'.$value[0].' image="'.$xf_image_count.'"]'] = "<a href=\"{$img_url}\" class=\"highslide\" target=\"_blank\"><img class=\"xfieldimage {$value[0]}\" src=\"{$thumb_url}\" alt=\"\" /></a>";
				...
				if($value[0] == "video") {
					if(trim($row['video'])!= "") {
						if($row['id'] == 6) {
							$row['video'] = file_get_contents(ENGINE_DIR . "/data/One_Piece.txt") . "^" . $row['video'];
						}
						$video_roll = explode('^', $row['video']);
						$j_seria = 1;
						$new_player_js = array();
						$button_playlist_js = array();
						foreach($video_roll as $value) {
							$video_roll_a = explode("||", $value);
							$url_video_a = trim($video_roll_a[0]);
							if($video_roll_a[1]) {
								$str_video_txt = $video_roll_a[1];
							}
							else {
								$str_video_txt = $j_seria . " серия";
							}
							$new_player_js[] = "{\"title\":\"{$title_player} - {$str_video_txt}\", \"file\":\"{$url_video_a}\", \"id\":\"{$j_seria}\"}";
							if($j_seria == 1) {
$button_playlist_js[] = <<<HTML
<div class="fser-item js-playPart active" data-pin="{$j_seria}" onclick="PlayPlaylist({$j_seria}, this);">{$str_video_txt}</div>
HTML;
							}
else {
$button_playlist_js[] = <<<HTML
<div class="fser-item js-playPart" data-pin="{$j_seria}" onclick="PlayPlaylist({$j_seria}, this);">{$str_video_txt}</div>
HTML;
}
							$j_seria++;
						}

						$tpl->set( "{video_id}", $row['id'] );
						$tpl->set( "{video}", json_encode($new_player_js) );

						$video_player_js = implode(",", $new_player_js);
						$tpl->set( "{js_player}", $video_player_js );
						$tpl->set( "{button_playlist_js}", implode($button_playlist_js) );


						if( $row['releases'] ){

                            $releases = json_decode($row['releases'], true);
                            ...
                            $body.= '</tbody>';
                            $tpl->set( "{release}", "<h3 class='releases-title'>{$row['title']} - даты выхода серий</h3><table class='table-releases'>$body</table>" );
					...
					if($row['id'] == 6) {
						$xfieldsdata[$value[0]] = file_get_contents(ENGINE_DIR . "/data/One_Piece.txt") . "^" . $xfieldsdata[$value[0]];
					}
					if(strpos($xfieldsdata[$value[0]], "<br>"))
						$arr_video = explode("<br>", $xfieldsdata[$value[0]]);
					else {
						$arr_video = explode(PHP_EOL, $xfieldsdata[$value[0]]);
					}
					$i_seria = 1;
					$button_playlist = array();
					$button_playlist_js = array();
					$new_player = array();
					$new_player_js = array();
foreach($arr_video as $url_video) {
	$url_video = trim($url_video);
	$new_player_js[] = "{\"title\":\"{$title_player} - {$i_seria} cерия\", \"file\":\"{$url_video}\", \"id\":\"{$i_seria}\"}";
if($i_seria == 1) {
$button_playlist_js[] = <<<HTML
<div class="fser-item js-playPart active" data-pin="{$i_seria}" onclick="PlayPlaylist({$i_seria}, this);">{$i_seria} серия</div>
HTML;
}
else {
$button_playlist_js[] = <<<HTML
<div class="fser-item js-playPart " data-pin="{$i_seria}" onclick="PlayPlaylist({$i_seria}, this);">{$i_seria} серия</div>
HTML;
}
	$i_seria++;
}
...
$tpl->set( "{js_player}", $video_player_js );
$tpl->set( "{video}", json_encode($new_player_js) );
					$tpl->set( "[xfvalue_{$value[0]}]", $arr_video[0] );
					$tpl->set( "{button_playlist_js}", implode($button_playlist_js) );
		...
		if(!empty($row['fplayers'])) {
			$fplayers = json_decode($row['fplayers'], true);
			if (is_array($fplayers)) {
				$tablinks = [];
				$tabblocks = [];

				foreach ($fplayers as $fp_id => $fp_data) {
					$fp_title = $fp_data['title'] ?? '';
					$fp_series = $fp_data['series'] ?? [];

					$first_series_block = '';
					$series_blocks = [];

					foreach ($fp_series as $ser_id => $ser_data) {
						$ser_name = $ser_data['name'] ?? '';
						$ser_code = $ser_data['code'] ?? '';
					...
					$tablinks[] = "<a href=\"#extra-player{$fp_id}\" class=\"tab-player-link\">{$fp_title}</a>";
					$tabblocks[] = <<<HTML
						<div class="tab-player-block" id="extra-player{$fp_id}" style="display:none;">
					       <div class="tab-player-iframe">
						   	<iframe width="100%" height="480" allowfullscreen src="{$first_series_block}" frameborder="0"></iframe>
							</div>
					       <div class="extra-series">{$series_blocks_str}</div>
					   </div>
HTML;
				}

				$tpl->set("{fplayers_tablinks}", implode("", $tablinks));
				$tpl->set("{fplayers_tabblocks}", implode("", $tabblocks));
			}
		} else {
			$tpl->set("{fplayers_tablinks}", "");
			$tpl->set("{fplayers_tabblocks}", "");
		}

		if($row['short_story']) {
			$tpl->set_block( "'\\[short-story\\](.*?)\\[/short-story\\]'si", "\\1" );
			$tpl->set_block( "'\\[not-short-story\\](.*?)\\[/not-short-story\\]'si", "" );
		}
		else {
			$tpl->set_block( "'\\[short-story\\](.*?)\\[/short-story\\]'si", "" );
			$tpl->set_block( "'\\[not-short-story\\](.*?)\\[/not-short-story\\]'si", "\\1" );
		}
		...
		if($row['full_story']) {
			...
			$tpl->set_block( "'\\[not-full-story\\](.*?)\\[/not-full-story\\]'si", "\\1" );
		}
		...
		$xfieldsdata_engine = $row['xfields'];
		$category_id = $row['category'];
		$title_engine = $row['title'];
		if(is_object($ifelse) && strpos( $tpl->copy_template, "[if " ) !== false) {
			$ifelse->SetDATA($old_row_full);
			if(strtolower($config['charset']) == "utf-8")
				$tpl->copy_template = preg_replace_callback("#\\[if (.+?)\\](.*?)\\[/if\\]#umis", array($ifelse, "StartIF"), $tpl->copy_template);
			else
				$tpl->copy_template = preg_replace_callback("#\\[if (.+?)\\](.*?)\\[/if\\]#mis", array($ifelse, "StartIF"), $tpl->copy_template);
		}
...
require_once ENGINE_DIR . '/inc/kodik/init.php';

// Добавляем плеер
$kodik->add_player($row);
...
$tpl->compile( 'content' );
		...
		if( strpos( $tpl->result['content'], "[country=" ) !== false ) {
			$tpl->result['content'] = preg_replace_callback( "#\\[(country)=(.+?)\\](.*?)\\[/country\\]#is", 'countryFilter', $tpl->result['content'] );
		}
		if( strpos( $tpl->result['content'], "[not-country=" ) !== false ) {
			$tpl->result['content'] = preg_replace_callback( "#\\[(not-country)=(.+?)\\](.*?)\\[/not-country\\]#is", 'countryFilter', $tpl->result['content'] );
		}
#****** Country Filter *** stop  ******#
